    ...
    <section ref={ref} className={cn("py-12 md:py-16 reveal", visible && "is-visible")}>
      <div className="container-page">
        <div className="text-center max-w-3xl mx-auto mb-10 md:mb-12 space-y-3">
          <span className="cr-chip">Explora por categorías</span>
          <h2 className="font-display text-4xl md:text-5xl font-bold leading-tight tracking-tight text-slate-900">
            Encuentra exactamente <span className="cr-shimmer-text">lo que necesitas</span>
          </h2>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {cats.map((c, i) => {
            ...
            const empty = n === 0;
            const card = cn(
              "group relative aspect-[4/5] rounded-3xl overflow-hidden shadow-elevated cr-sheen cr-lift",
              "reveal",
              visible && "is-visible",
            );
            const body = (
              <>
                {cover && !empty ? (
                  ...
                  <div className="absolute inset-0 bg-gradient-to-br from-blue-100 via-slate-100 to-cyan-100 flex items-center justify-center">
                    <Headphones className="w-20 h-20 text-blue-400/50 group-hover:text-blue-500/70 group-hover:scale-110 transition-all duration-500" />
                  </div>
                )}
