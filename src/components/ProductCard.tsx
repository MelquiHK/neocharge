...
function ProductCardComponent({ product, variant = "default", isFavorite: propIsFavorite, onToggleFavorite: propOnToggleFavorite }: ProductCardProps) {
  ...
  return (
    <div
      className={cn(
        "group relative rounded-[1.5rem] overflow-hidden transition-all duration-700 bg-[#0d0d16] border border-white/15 nc-liquid lift nc-shine-hover flex flex-col h-full",
      ...
      <div className="relative flex flex-col flex-1">
        ...
        <div className="relative">
          <Link to={productLink} className="block" aria-label={product.name}>
            <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-[#15151f] to-[#08080d]">
              {/* Reflejo suave estático tipo "agua" sobre la foto */}
              <div className="absolute inset-x-0 top-0 h-20 bg-gradient-to-b from-white/10 via-white/5 to-transparent pointer-events-none z-[5]" />
              {/* Animated background glow */}
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700">
                <div className="absolute inset-0 bg-gradient-to-br from-lime-400/10 via-transparent to-violet-600/10"></div>
              </div>
              ...
              <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
                {discount && (
                  <div className="px-3 py-1.5 rounded-lg bg-[#a3e635] text-[#0c0c14] text-xs font-bold shadow-lifted animate-bounce-in">
                    -{discount}%
                  </div>
                )}
              ...
              {outOfStock && (
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/40 to-transparent backdrop-blur-sm flex items-center justify-center z-20">
          ...
          <div className="absolute bottom-3 left-3 right-3 rounded-2xl border border-white/20 nc-liquid-soft p-2 translate-y-[130%] group-hover:translate-y-0 [@media(hover:none)]:translate-y-0 transition-transform duration-500 z-30 pointer-events-auto shadow-xl">
            <Button
              type="button"
              onClick={handleAdd}
              disabled={outOfStock}
              className={cn(
                "w-full font-bold rounded-xl shadow-xl relative overflow-hidden h-12 active:scale-[0.98] transition-all",
                !added && "nc-btn-shine",
                added
                  ? "bg-green-500 text-white"
                  : "bg-[#a3e635] text-[#0c0c14] hover:bg-[#bef264]"
              )}
            >
              {added ? (
        ...
        <div className="p-4 flex flex-col flex-1 gap-3">
          ...
          <div className="flex items-start gap-2">
            <Link to={productLink} className="flex-1 min-w-0">
              <h3 className="font-display font-bold text-lg leading-tight text-white group-hover:text-lime-300 transition-colors line-clamp-2">
                {product.name}
              </h3>
            </Link>
            <button
              ...
              className={cn(
                "shrink-0 p-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/15 transition-all duration-300 hover:scale-110 active:scale-95",
          ...
          {product.stock > 0 && product.stock <= 5 && (
            <div className="p-2 rounded-lg bg-gradient-to-r from-orange-500/20 to-red-500/20 border border-orange-400/40">
          ...
          {product.stock > 0 && (
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-400">Stock disponible</span>
              ...
              >
                <div
                  className="h-full bg-gradient-to-r from-lime-400 to-violet-500 transition-all duration-500"
                  style={{
