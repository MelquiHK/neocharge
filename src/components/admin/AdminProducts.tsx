...
import { Copy, Download, Plus, Pencil, Trash2, Image as ImageIcon, X, Star, Sparkles, CheckSquare, Square, ArrowUp, ArrowDown } from "lucide-react";
...
export function AdminProducts() {
  ...
  const [sortBy, setSortBy] = useState<"position" | "newest" | "name" | "price" | "stock">("position");
  ...
  const moveProduct = async (id: string, dir: "up" | "down") => {
    ...
    if (otherIdx < 0 || otherIdx >= filtered.length) return;
    const a = filtered[idx];
    const b = filtered[otherIdx];
    const aOrder = a.sort_order ?? idx;
    const bOrder = b.sort_order ?? otherIdx;
    const { error } = await supabase.from("products").update({ sort_order: bOrder } as any).eq("id", a.id);
    if (error) return toast.error(error.message);
    const { error: e2 } = await supabase.from("products").update({ sort_order: aOrder } as any).eq("id", b.id);
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 w-full">
          ...
          <div className="space-y-1">
            ...
            <Select value={sortBy} onValueChange={(v) => setSortBy(v as any)}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="position">Posición en tienda</SelectItem>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr className="text-left text-xs uppercase text-muted-foreground">
                ...
                <th className="py-3 px-4 w-24">Posición</th>
            ...
            <tbody>
                  ...
                  <tr key={p.id} className="border-t border-border">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex flex-col items-center gap-1">
                        <span className="text-xs font-bold text-primary">#{p.sort_order ?? 0}</span>
                        <div className="flex gap-0.5">
                          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => moveProduct(p.id, "up")} title="Subir">
                            <ArrowUp className="w-3.5 h-3.5" />
                          </Button>
                          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => moveProduct(p.id, "down")} title="Bajar">
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="py-8 text-center text-muted-foreground">No hay productos. Crea el primero con "Nuevo producto".</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {editing && (
            <div className="space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label>Posición en tienda</Label>
                  <Input
                    type="number"
                    min={0}
                  ...
                  />
                  <p className="text-xs text-muted-foreground">Menor número = aparece primero. Los destacados siempre van al inicio de la tienda.</p>
                </div>
                <div className="space-y-2">
                  <Label>Categoría</Label>
