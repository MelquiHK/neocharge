    ...
    return (
      <div className="cr-page">
        ...
        <div className="relative container-page py-20 space-y-4">
          <div className="h-8 bg-blue-100/60 rounded animate-pulse w-1/3" />
          <div className="h-4 bg-blue-100/60 rounded animate-pulse w-1/2" />
          <div className="space-y-3 mt-8">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="h-20 bg-blue-100/60 rounded-3xl animate-pulse" />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="cr-page">
      ...
      <div className="relative container-page py-12 space-y-8">
        <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 border-b border-blue-100/60 pb-10">
          <div className="space-y-3">
            ...
            <h1 className="font-display text-5xl font-bold tracking-tight cr-shimmer-text">Mi cuenta</h1>
            <p className="text-lg text-slate-600 font-light">{user.email}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            {isAdmin && (
              <Button asChild className="cr-btn-ghost">
                <Link to="/admin"><LayoutDashboard className="w-4 h-4" /> Panel admin</Link>
              </Button>
            )}
            {isMensajero && (
              <Button asChild className="cr-btn">
                <Link to="/mensajeria"><Map className="w-4 h-4" /> Panel Mensajero</Link>
              </Button>
            )}
            <Button onClick={() => setEditing(!editing)} className="cr-btn-ghost">
              {editing ? "Cancelar" : "Editar perfil"}
            </Button>
            <Button onClick={signOut} className="rounded-full bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 font-bold">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 space-y-6">
            <section className="rounded-[2rem] cr-glass cr-sheen p-6 space-y-4">
              <div className="flex flex-col items-center text-center space-y-3">
                <div className="w-24 h-24 rounded-full bg-blue-100/60 flex items-center justify-center overflow-hidden border-4 border-white/80 shadow-lg">
                  {profile?.avatar_url ? (
                ...
                <div>
                  <h3 className="font-display text-xl font-bold text-slate-900">{profile?.full_name || profile?.username}</h3>
              ...
              <div className="space-y-3 pt-4 border-t border-blue-100/60">
                ...
                <div className="flex items-start gap-2 text-sm">
                  ...
                  <p className="text-slate-500 italic">{(profile as ProfileWithBio | null)?.bio || "Sin biografía"}</p>
                </div>
              </div>
            </section>

            {editing && (
              <section className="rounded-[2rem] cr-glass cr-sheen p-6 space-y-4 animate-in fade-in slide-in-from-top-4">
                <h3 className="font-bold flex items-center gap-2 text-slate-900"><Save className="w-4 h-4" /> Editar Datos</h3>
                <div className="space-y-3">
                  <div className="space-y-1">
                    <Label className="text-slate-700 font-medium">Nombre Completo</Label>
                    <Input value={formData.full_name} onChange={e => setFormData({...formData, full_name: e.target.value})} className="cr-input h-12 rounded-2xl" />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-slate-700 font-medium">Nombre de Usuario</Label>
                    <Input value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} className="cr-input h-12 rounded-2xl" />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-slate-700 font-medium">Teléfono</Label>
                    <Input value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="cr-input h-12 rounded-2xl" />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-slate-700 font-medium">Foto de Perfil</Label>
                    <div className="flex items-center gap-4">
                      <div className="w-20 h-20 rounded-full bg-blue-100/60 flex items-center justify-center overflow-hidden border border-white/80">
                        {selectedFile ? (
                      ...
                      <Input
                        ...
                        className="cr-input flex-grow h-12 rounded-2xl file:text-slate-600"
                      />
                    </div>
                    {selectedFile && (
                  ...
                  <div className="space-y-1">
                    ...
                    <Textarea value={formData.bio} onChange={e => setFormData({...formData, bio: e.target.value})} className="cr-input rounded-2xl" />
                  </div>
                  <Button onClick={handleSaveProfile} disabled={saving || uploading} className="w-full mt-4 h-12 cr-btn disabled:translate-y-0">
          ...
          <div className="lg:col-span-2 space-y-6">
            {isGestor && gestorStats && (
              <section className="rounded-[2rem] cr-glass cr-sheen p-6">
                <div className="flex items-center justify-between flex-wrap gap-3 mb-6">
                  <h2 className="font-display text-2xl font-bold flex items-center gap-2 text-slate-900">
                    <Wallet className="w-6 h-6 text-blue-500" /> Resumen de Gestor
                  </h2>
                  <div className="flex gap-2">
                    <Button size="sm" onClick={sendToWhatsApp} className="rounded-full bg-green-100 border border-green-200 text-green-700 hover:bg-green-200 font-bold">
                      <MessageSquare className="w-4 h-4 mr-2" /> WhatsApp Admin
                    </Button>
                    <Button size="sm" onClick={handleRequestPayment} disabled={requestingPayment} className="cr-btn disabled:translate-y-0">
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  <div className="cr-glass-soft p-4 rounded-2xl">
                    <div className="flex items-center gap-2 text-xs font-medium uppercase text-slate-500 mb-1">
                  ...
                  <div className="cr-glass-soft p-4 rounded-2xl">
                    ...
                    <div className="text-2xl font-bold text-blue-600">{gestorStats.bySeller[0]?.count || 0}</div>
                  </div>
                </div>
              </section>
            )}
            ...
            <section className="rounded-[2rem] cr-glass cr-sheen p-6">
              <h2 className="font-display text-xl font-bold mb-4 flex items-center gap-2 text-slate-900">
                <Package className="w-5 h-5 text-blue-500" /> Mis pedidos
              </h2>
              {orders.length === 0 ? (
                <div className="text-center py-12 space-y-3">
                  <p className="text-slate-500">No tienes pedidos todavía.</p>
                  <Button asChild className="cr-btn px-6 h-12"><Link to="/tienda">Empezar a comprar</Link></Button>
                </div>
              ) : (
                <div className="divide-y divide-blue-100/60">
                  {orders.map((o) => (
                    <div key={o.id} className="py-4 flex items-center justify-between gap-4">
                      <div>
                        <p className="font-semibold text-sm text-slate-900">Pedido #{o.id.slice(0, 8)}</p>
                        <p className="text-xs text-slate-500">
                          {new Date(o.created_at).toLocaleString("es-CU")} · {Array.isArray(o.items) ? o.items.length : 0} productos
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="font-display font-bold text-blue-600">{formatPrice(o.total)}</p>
                        <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 border border-blue-100 text-slate-600 capitalize">{o.status}</span>
                      </div>
                    </div>
                  ))}
                </div>
